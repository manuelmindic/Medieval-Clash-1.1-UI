...
public class GameBot : MonoBehaviour
{
    ...
    public bool checkIfWon(User user1, User user2, int turn)
    {
        if (user1.HealthPoints <= 0)
        {
            ...
            WriteToFileMetrics(metricsState);
            return true;
        }
    ...
    public void WriteToFileMetrics(MetricsState metricsState)
    {
        string json = JsonUtility.ToJson(metricsState, true);

        // Desired directory and file path
        ...
        Debug.Log("GameState saved to: " + filePath);
    }
